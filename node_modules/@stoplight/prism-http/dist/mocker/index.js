...
const mock = ({ resource, input, config, }) => {
    const payloadGenerator = config.dynamic
        ? (0, lodash_1.partial)(JSONSchema_1.generate, resource['__bundled__'])
    ...
    return (0, function_1.pipe)((0, withLogger_1.default)(logger => {
        ...
        return config;
    }), R.chain(mockConfig => negotiateResponse(mockConfig, input, resource)), R.chain(result => negotiateDeprecation(result, resource)), R.chain(result => assembleResponse(result, payloadGenerator)), R.chain(response => logger => (0, function_1.pipe)(response, E.map(mockResponseLogger(logger)), E.map(response => runCallbacks({ resource, request: input.data, response })(logger)), E.chain(() => response))));
};
...
function parseBodyIfUrlEncoded(request, resource) {
    ...
    const encodedUriParams = (0, body_1.splitUriParams)(request.body);
    if (specs.length < 1) {
    ...
    return Object.assign(request, {
        body: (0, body_1.deserializeFormBody)(content.schema, encodings, (0, body_1.decodeUriEntities)(encodedUriParams)),
    });
}
...
const assembleResponse = (result, payloadGenerator) => logger => (0, function_1.pipe)(E.Do, E.bind('negotiationResult', () => result), E.bind('mockedData', ({ negotiationResult }) => eitherSequence(computeBody(negotiationResult, payloadGenerator), computeMockedHeaders(negotiationResult.headers || [], payloadGenerator))), E.map(({ mockedData: [mockedBody, mockedHeaders], negotiationResult }) => {
    const response = {
    ...
    return eitherRecordSequence((0, lodash_1.mapValues)((0, lodash_1.keyBy)(headers, h => h.name), header => {
        if (header.schema) {
            if (header.examples && header.examples.length > 0) {
                ...
                return (0, function_1.pipe)(payloadGenerator(header.schema), E.map(example => {
