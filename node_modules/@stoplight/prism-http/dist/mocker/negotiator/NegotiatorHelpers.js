...
const helpers = {
    negotiateByPartialOptionsAndHttpContent({ code, exampleKey, dynamic }, httpContent) {
        ...
        if (exampleKey) {
            return (0, function_1.pipe)((0, InternalHelpers_1.findExampleByKey)(httpContent, exampleKey), E.fromOption(() => types_1.ProblemJsonError.fromTemplate(errors_1.NOT_FOUND, `Response for contentType: ${mediaType} and exampleKey: ${exampleKey} does not exist.`)), E.map(bodyExample => ({ code, mediaType, bodyExample })));
        }
        else if (dynamic) {
            return (0, function_1.pipe)(httpContent.schema, E.fromNullable(new Error(`Tried to force a dynamic response for: ${mediaType} but schema is not defined.`)), E.map(schema => ({ code, mediaType, schema })));
        }
        else {
            return E.right((0, function_1.pipe)((0, InternalHelpers_1.findFirstExample)(httpContent), O.map(bodyExample => ({ code, mediaType, bodyExample })), O.alt(() => (0, function_1.pipe)(O.fromNullable(httpContent.schema), O.map(schema => ({ schema, code, mediaType })))), O.getOrElse(() => ({ code, mediaType }))));
        }
    },
