...
const validateParams = (target, specs, bundle) => ({ deserializers, prefix, defaultStyle }) => {
    ...
    }), O.chain(({ parameterValues, schema }) => (0, utils_1.validateAgainstSchema)(parameterValues, schema, true, prefix, bundle)), O.map(schemaDiagnostic => NEA.concat(schemaDiagnostic, deprecatedWarnings)), O.alt(() => NEA.fromArray(deprecatedWarnings)), E.fromOption(() => target), E.swap);
};
